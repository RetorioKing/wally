...
with tabs[1]:
    ...
    if st.button("Run Classification") and features:
        ...
        selected = st.selectbox("Choose model", list(clf_results.keys()))
        cm = clf_results[selected]["confusion_matrix"]
        fig, ax = plt.subplots()
        ...
        pred_file = st.file_uploader("Upload data without target", type=["xlsx"])
        if pred_file:
            pred_df = pd.read_excel(pred_file)
            preds = clf_results[selected]["model"].predict(pred_df)
            ...
            st.download_button(
                "Download Predictions", buf.getvalue(),
...
with tabs[2]:
    ...
    clust_feats = st.multiselect(
        "Select features", num_cols,
        default=list(num_cols[:4])
    )
    n_clusters = st.slider("Number of clusters", 2, 10, 3)

    if st.button("Run K-Means") and clust_feats:
        labels, inertia, _ = run_kmeans(df[clust_feats], n_clusters)
        df["Cluster"] = labels
        ...
        st.download_button(
            "Download Clustered Data", buf.getvalue(),
...
with tabs[3]:
    ...
    if len(cat_cols) >= 2:
        ...
        col2 = st.selectbox("Column 2", cat_cols, index=1)
...
with tabs[4]:
    ...
    if len(num_cols) >= 2:
        target_r = st.selectbox("Target", num_cols)
        feat_r = st.multiselect(
            "Features", num_cols.drop(target_r),
